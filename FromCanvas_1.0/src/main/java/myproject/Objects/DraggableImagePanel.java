...
public class DraggableImagePanel extends JPanel {
    ...
    public DraggableImagePanel(int posX, int posY, String imgPath, Color clr1, Color clr2, int width, int height) {
        ...
        // Если ширина и высота не заданы (<= 0), используем размер по умолчанию
        ...
        if (imgPath != null && !imgPath.isEmpty()) {
            ...
            // Если размеры НЕ были заданы явно (width <= 0 или height <= 0), подбираем оптимальный размер
            if (width <= 0 || height <= 0) {
    ...
    private void openImageChooser() {
        ...
        if (fileChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
            ...
            // При смене изображения, если текущий размер был автоматическим, корректируем
            // Для простоты всегда корректируем размер под новое изображение
    ...
    private void enterEditMode() {
        ...
        if (MyFrame.currentlyEditingImagePanel == this) {
            return;
        }
        MyFrame.beginImageEditing(this);
        editing = true;
    ...
    public void exitEditMode() {
        if (MyFrame.currentlyEditingImagePanel == this) {
            MyFrame.currentlyEditingImagePanel = null;
