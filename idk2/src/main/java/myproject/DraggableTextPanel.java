...
public class DraggableTextPanel extends JPanel {
    ...
    private boolean dragging = false, resizing = false, mouseOver = false, editing = false;
    ...
    public DraggableTextPanel(Integer posX, Integer posY, int width, int height, String text, Color clr1, Color clr2, TextAlignment alignment) {
        ...
        MyFrame.BackgroundPanel.setLayer(this,DEFAULT_LAYER);
        Point pos = MyFrame.getRandomVisiblePosition(180, 180);
        ...
        // Добавляем слушатели мыши к текстовой области для перехвата событий
        textArea.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) enterEditMode();
                else {
                    // Одиночный клик передаем родительской панели
            ...
            @Override public void mousePressed(MouseEvent e) {
                // Всегда передаем событие нажатия родительской панели
                MouseEvent parentEvent = SwingUtilities.convertMouseEvent(textArea, e, DraggableTextPanel.this);
                DraggableTextPanel.this.dispatchEvent(parentEvent);
            }
    ...
    private void initTextArea(String text) {
        ...
        // Делаем textArea прозрачной для событий мыши в режиме просмотра
    ...
    private void addMouseListeners() {
        addMouseListener(new MouseAdapter() {
            @Override public void mousePressed(MouseEvent e) {
                ...
                // Если в режиме редактирования и клик в зоне изменения размера
                ...
                // Если НЕ в режиме редактирования (режим просмотра) - можно перетаскивать отовсюду
            ...
            @Override public void mouseReleased(MouseEvent e) {
                if (dragging || resizing) {
                    ...
                    // Если вышли из режима редактирования, устанавливаем курсор перемещения
            ...
            @Override public void mouseEntered(MouseEvent e) {
                mouseOver = true;
                // В режиме просмотра показываем курсор перемещения
                if (!textArea.isEnabled()) {
                    setCursor(Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR));
                }
                repaint();
            ...
            @Override public void mouseExited(MouseEvent e) {
                mouseOver = false;
                // Только если не в процессе перетаскивания или изменения размера
                if (!dragging && !resizing) {
                    setCursor(Cursor.getDefaultCursor());
                }
                repaint();
            ...
            @Override public void mouseClicked(MouseEvent e) {
                // Одиночный клик в режиме просмотра ничего не делает
                // Двойной клик обрабатывается в textArea
                e.consume();
            }
        });
        ...
        addMouseMotionListener(new MouseAdapter() {
            ...
            @Override public void mouseMoved(MouseEvent e) {
                ...
                if (textArea.isEnabled() && isNearResizeZone(point)) {
                    ...
                    // В режиме просмотра везде показываем курсор перемещения
                    setCursor(Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR));
                }
                else if (textArea.isEnabled() && !isNearResizeZone(point)) {
                    // В режиме редактирования вне зоны изменения размера - обычный курсор
                    setCursor(Cursor.getDefaultCursor());
                }
            }
        });
    ...
    MyFrame.BackgroundPanel.setLayer(this,10);
    if (MyFrame.currentlyEditingPanel == this) {
        return;
    }
    
   
    if (MyFrame.currentlyEditingPanel != null) {
        MyFrame.currentlyEditingPanel.exitEditMode();
    }
    
    MyFrame.currentlyEditingPanel = this;
    MyFrame.IsEditing = true;
    ...
    if (MyFrame.currentlyEditingPanel == this) {
        MyFrame.BackgroundPanel.setLayer(this,DEFAULT_LAYER);
        MyFrame.currentlyEditingPanel = null;
        MyFrame.IsEditing = false;
    }
    ...
    private void drawResizeHandle(Graphics2D g2) {
        ...
        int cornerX = x + size - cornerSize - 2;
        int cornerY = y + size - cornerSize - 2;
        
        //g2.drawLine(cornerX, cornerY + cornerSize, cornerX + cornerSize, cornerY);
