...
public class Arrow extends JPanel {
    ...
    Arrow(int x1, int y1, int x2, int y2, Point cnPoint, Color color, int headSize) {
        ...
        control = new Point((x1 + x2) / 2, (y1 + y2) / 2);
            ...
            control = cnPoint;
            // Проверяем, находится ли контрольная точка не в центре
    ...
    public void exitEdit() {
        if (MyFrame.currentlyEditingArrow == this) {
    ...
    public void enterEdit() {
        ...
        if (MyFrame.currentlyEditingArrow != null) {
            MyFrame.currentlyEditingArrow.exitEdit();
        }
    
        MyFrame.currentlyEditingArrow = this;
    ...
    private boolean isOnLine(Point p) {
        if (curved) {
            ...
            QuadCurve2D curve = new QuadCurve2D.Float(s.x, s.y, c.x, c.y, e.x, e.y);
